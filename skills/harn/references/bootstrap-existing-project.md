...
Before bootstrapping, establish a Git baseline.
...
If the repo is not a Git repo, initialize Git and make a baseline commit before running Harn unless the human explicitly says not to.

If the repo already has large uncommitted scaffold work, stop and ask whether to commit that baseline first. Do not create dirty locks to explain already-written scaffold work unless the human explicitly accepts that risk.
...
For frontend entities, include behavior and runtime contracts, not inert DOM facts. Good frontend assumptions may describe parallax behavior, script ordering, generated content mounts, runtime DOM ownership, component refresh rules, responsive behavior, accessibility contracts, route-state persistence, or old-script-to-new-component integration contracts.
...
The skill index rows and animated bars are generated by site.js at runtime.
```

Avoid assumptions that merely restate markup:

```txt
#graph exists.
There is a div for the hero.
#skillIndex stays empty.
```
...
Never edit a locked plan to fit implementation. If the locked plan is wrong, stop and create a replacement plan or ask the human whether to abandon/delete the bad plan.
